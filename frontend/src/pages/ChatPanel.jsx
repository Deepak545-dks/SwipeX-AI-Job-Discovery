...
export default function ChatPanel() {
  ...
  return (
    <PageTransition className="max-w-6xl mx-auto px-4 sm:px-6 py-12 text-white relative">
      
      {/* Background spotlights: Pink + Violet Theme for Messages Page */}
      <div className="absolute top-[10%] left-[15%] w-[420px] h-[420px] bg-gradient-to-tr from-pink-500/15 via-violet-605/10 to-transparent rounded-full blur-[125px] -z-10 pointer-events-none" />
      <div className="absolute bottom-[20%] right-[15%] w-[380px] h-[380px] bg-gradient-to-tr from-violet-600/15 via-pink-500/10 to-transparent rounded-full blur-[125px] -z-10 pointer-events-none" />

      {/* Messages layout frame */}
      <div className="rounded-[28px] border border-white/10 bg-slate-950/80 backdrop-blur-2xl grid lg:grid-cols-4 min-h-[75vh] max-h-[80vh] overflow-hidden shadow-2xl">
        
        {/* Sidebar Directory */}
        <div className={`lg:col-span-1 border-r border-white/10 flex flex-col h-full bg-slate-900/10 ${selectedRoom ? 'hidden lg:flex' : 'flex'}`}>
          <div className="p-4 sm:p-5 border-b border-white/10 text-left">
            <h2 className="text-lg font-black text-white flex items-center gap-2">
              <Cpu size={18} className="text-pink-400" /> Matches Inbox
            </h2>
            <p className="text-slate-400 text-[10px] uppercase font-black tracking-widest mt-1">Direct Match Lines</p>
          </div>
          ...
          <div className="flex-grow overflow-y-auto p-3 space-y-2">
            {loadingRooms ? (
              <div className="flex flex-col items-center py-10 space-y-2">
                ...
                <span className="text-[10px] text-slate-500 font-extrabold uppercase">Scanning channels...</span>
              </div>
            ) : rooms.length === 0 ? (
              <p className="text-slate-500 text-xxs py-10 text-center font-bold">No active conversations yet.</p>
            ) : (
              rooms.map((room) => {
                ...
                return (
                  <button
                    ...
                    className={`w-full flex items-center justify-between p-3.5 rounded-2xl transition-all text-left cursor-pointer border ${
                      isSelected 
                        ? 'bg-gradient-to-r from-pink-500 via-violet-605 to-indigo-650 text-white shadow-md border-pink-500/20' 
                        : 'hover:bg-white/5 text-slate-350 hover:text-white border-transparent'
                    }`}
                  >
                    <div className="flex items-center space-x-3 truncate">
                      <div className={`w-9 h-9 rounded-full flex items-center justify-center shrink-0 font-black text-xs relative ${
                        isSelected ? 'bg-white/20 text-white' : 'bg-slate-900 border border-white/10 text-pink-400'
                      }`}>
                      ...
                      <div className="truncate">
                        <p className={`text-xs font-black truncate ${isSelected ? 'text-white' : 'text-white'}`}>{opponent.name}</p>
                    ...
                    {room.unread_count > 0 && (
                      <span className={`px-2 py-0.5 rounded-md text-[8px] font-black shrink-0 ml-2 ${
                        isSelected ? 'bg-white text-pink-500' : 'bg-pink-600 text-white'
                      }`}>
                        {room.unread_count}
                      </span>
                    )}
        ...
        <div className={`lg:col-span-3 flex flex-col h-full bg-slate-950/10 ${selectedRoom ? 'flex' : 'hidden lg:flex'}`}>
          {selectedRoom ? (
            <>
              {/* Header details bar */}
              <div className="p-4 sm:p-5 border-b border-white/10 flex items-center justify-between bg-slate-900/20 text-left">
              ...
              <div className="flex-grow overflow-y-auto p-4 sm:p-6 space-y-5">
                {wsError && (
                  <div className="p-3.5 bg-rose-500/10 border border-rose-500/20 text-rose-400 text-xxs rounded-xl text-center flex items-center justify-center space-x-2">
                      ...
                      >
                        <div className="max-w-[70%] space-y-1 text-left">
                          <div className={`px-4.5 py-3 rounded-2xl text-xs leading-relaxed font-semibold shadow-sm ${
                            isMe 
                              ? 'bg-gradient-to-r from-pink-500 via-violet-605 to-indigo-650 text-white rounded-tr-none shadow-sm' 
                          ...
                          <div className={`flex items-center space-x-1.5 text-[9px] text-slate-500 font-bold uppercase tracking-wider ${
                            ...
                            {isMe && (
                              m.is_read ? <CheckCheck size={12} className="text-pink-400" /> : <Check size={12} />
                            )}
                          </div>
                        </div>
                      </div>
              ...
              <form onSubmit={handleSendMessage} className="p-4 border-t border-white/10 bg-slate-900/30 flex items-center gap-2">
                ...
                <button
                  ...
                  className="p-3.5 bg-gradient-to-r from-pink-500 to-violet-600 hover:from-pink-400 hover:to-violet-550 text-white rounded-2xl transition-all shadow-md active:scale-95 disabled:opacity-30 disabled:cursor-not-allowed shrink-0 cursor-pointer"
                  title="Send Message"
                >
                  <Send size={15} />
                </button>
