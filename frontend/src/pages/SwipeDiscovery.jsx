...
import { 
  ...
  Briefcase, GraduationCap, FileText, ChevronUp, AlertCircle, CheckCircle, Sparkles, Info, Building, Check, RefreshCw, Copy, Keyboard, Activity, ShieldAlert
} from 'lucide-react';
    ...
    try {
      ...
      if (action === 'like') {
        if (response.data.applied) {
          ...
          showToast('Job liked successfully!', 'success');
        }
      } else if (action === 'save') {
  ...
  const handleUndo = async () => {
    try {
      ...
      if (response.data.job) {
        ...
        showToast('Last swipe undone successfully.', 'success');
      }
    } catch (err) {
      ...
      <PageTransition className="max-w-6xl mx-auto px-6 py-12 flex items-center justify-center">
        <div className="w-full max-w-sm h-[540px] bg-slate-900/40 border border-white/5 rounded-3xl p-6 flex flex-col justify-between backdrop-blur-xl shadow-2xl relative overflow-hidden">
          <div className="space-y-6 animate-pulse">
            <div className="h-6 rounded bg-white/5 w-1/3" />
            <div className="h-10 rounded bg-white/5 w-3/4" />
            <div className="h-20 rounded bg-white/5 w-full" />
          </div>
        </div>
      </PageTransition>
    );
  ...
  return (
    <PageTransition className="max-w-6xl mx-auto px-6 py-12 grid lg:grid-cols-12 gap-8 relative z-10">
      ...
      <div className="lg:col-span-8 flex flex-col items-center justify-center">
        {deck.length === 0 ? (
          <div className="text-center p-10 bg-slate-950/80 border border-violet-500/20 rounded-3xl max-w-md w-full backdrop-blur-2xl shadow-2xl space-y-6">
            <div className="w-16 h-16 mx-auto rounded-2xl bg-gradient-to-tr from-violet-600 to-fuchsia-600 shadow-lg flex items-center justify-center text-white">
            ...
            {!hasResume && (
              <div className="p-4 bg-amber-955/20 border border-amber-500/20 rounded-2xl text-xxs text-amber-400 text-left flex items-start space-x-2.5">
            ...
            <div className="flex flex-col gap-3 pt-2">
              <button
                ...
                disabled={resetting}
                className="flex items-center justify-center space-x-2 py-3.5 px-4 bg-gradient-to-r from-violet-600 via-fuchsia-600 to-cyan-600 hover:from-violet-500 hover:via-fuchsia-500 hover:to-cyan-500 text-white rounded-2xl text-xs font-extrabold transition-all shadow-lg active:scale-95 disabled:opacity-50 cursor-pointer uppercase tracking-wider"
              >
              ...
              <div className="grid grid-cols-2 gap-2">
                <Link
                  to="/profile"
                  className="flex items-center justify-center py-3 bg-white/5 hover:bg-white/10 text-slate-350 hover:text-white rounded-2xl text-xs font-bold transition-all border border-white/10 active:scale-95 uppercase tracking-wider text-center"
                >
                  Profile
                </Link>
                <button
          ...
          <div className="relative w-full max-w-sm h-[640px] flex flex-col justify-between items-center">
                ...
                {deck.slice(0, 3).reverse().map((job, idx, arr) => {
                  ...
                  return (
                    <motion.div
                      ...
                      className="absolute w-full h-full p-[1.5px] rounded-[32px] overflow-hidden bg-gradient-to-tr from-violet-500/25 via-fuchsia-500/25 to-cyan-500/25 shadow-2xl"
                    >
                      <div className="w-full h-full bg-slate-950/95 backdrop-blur-2xl rounded-[29px] p-6 flex flex-col justify-between cursor-grab active:cursor-grabbing select-none relative overflow-hidden">
                        ...
                        <div>
                          ...
                          <div className="space-y-2 mt-6 border-t border-white/10 pt-4 text-left">
                          ...
                          <div className="mt-6 pt-3 border-t border-white/10 text-left">
                            ...
                            <div className="flex flex-wrap gap-1.5 mt-2.5 max-h-[85px] overflow-hidden">
                              ...
                              {job.skills_required.length > 4 && (
                                <span className="px-2 py-1 text-slate-550 text-xxs font-black">
            ...
            <div className="flex items-center space-x-5 z-10 pt-4">
              <button
                onClick={handleUndo}
                className="w-12 h-12 rounded-full bg-slate-900 border border-white/10 hover:border-white/20 hover:bg-white/5 flex items-center justify-center text-slate-500 hover:text-slate-350 transition-all shadow-lg active:scale-90 cursor-pointer"
              ...
              <button
                ...
                className="w-15 h-15 rounded-full bg-slate-900 border border-rose-500/20 hover:border-rose-500/60 hover:bg-rose-955/20 flex items-center justify-center text-rose-500 shadow-xl hover:scale-105 active:scale-90 transition-all cursor-pointer"
              ...
              <button
                ...
                className="w-13 h-13 rounded-full bg-slate-900 border border-cyan-500/25 hover:border-cyan-500/60 hover:bg-cyan-955/20 flex items-center justify-center text-cyan-400 shadow-lg hover:scale-105 active:scale-90 transition-all cursor-pointer"
                title="Save (Up Arrow)"
              >
                <Star size={20} />
              </button>
              ...
              <button
                ...
                className="w-15 h-15 rounded-full bg-gradient-to-r from-violet-600 to-indigo-650 hover:from-violet-500 hover:to-indigo-550 flex items-center justify-center text-white shadow-xl shadow-indigo-650/30 hover:scale-105 active:scale-90 transition-all cursor-pointer"
      ...
      <div className="lg:col-span-4 space-y-6">
        <div className="p-[1px] rounded-3xl bg-gradient-to-b from-white/10 to-transparent shadow-xl">
          <div className="bg-slate-950/80 backdrop-blur-2xl rounded-[23px] p-6 space-y-6 text-left">
            ...
            <div className="space-y-4 max-h-[360px] overflow-y-auto divide-y divide-white/5">
              {[
                ...
                <div key={i} className="pt-3 first:pt-0 text-[11px] leading-relaxed">
                  ...
                  <p className="text-slate-400 font-medium mt-1">{activity.desc}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="p-[1.5px] rounded-3xl bg-gradient-to-tr from-violet-500/20 to-cyan-500/25 shadow-xl">
          <div className="bg-slate-950/80 backdrop-blur-2xl rounded-[23px] p-6 text-left space-y-4">
            <h4 className="text-xs font-extrabold uppercase tracking-widest text-slate-500">Your Swipe Stats</h4>
            <div className="grid grid-cols-2 gap-4">
              <div className="p-3 bg-white/5 border border-white/5 rounded-xl">
                <span className="text-xs font-black text-white block">12</span>
                <span className="text-[9px] text-slate-500 font-extrabold uppercase tracking-wider mt-1 block">Active Likes</span>
              </div>
              <div className="p-3 bg-white/5 border border-white/5 rounded-xl">
                <span className="text-xs font-black text-white block">4</span>
                <span className="text-[9px] text-slate-500 font-extrabold uppercase tracking-wider mt-1 block">Matches Found</span>
              </div>
            </div>
          </div>
        </div>
                ...
                {coverLetter && (
                  <div className="space-y-2 pt-2 border-t border-white/5 animate-fade-in text-left">
                    ...
                    <textarea
                      ...
                      rows={6}
                      className="w-full p-4 rounded-xl bg-slate-955/70 border border-white/10 text-slate-300 text-xs leading-relaxed outline-none focus:border-violet-500/50 focus:ring-4 focus:ring-violet-500/5 transition-all font-semibold"
                    />
                  </div>
                )}
              </div>
